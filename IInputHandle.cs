...
public interface IInputHandle
{
    ...
    // 3. 공격 상태 진입 (예: A 키 입력)
    bool IsAttackModeRequested();

    // 4. 공격 상태 중, 좌클릭으로 타겟 선택
    bool TryGetAttackTarget(out Transform target);
    bool TryGetAttackGroundPosition(out Vector3 groundPos);

    // 5. 스킬 키 바인딩 처리 (QWER 키 입력 확인)
    bool TryGetSkillKeyDown(out SkillType skillType);
